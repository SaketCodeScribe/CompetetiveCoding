...
class Solution3 {
    ...
    private void dfs(int u, int[] present, int[] future, List<Integer>[] tree,
        ...
        for (int v : tree[u]) {
            ...
            childDPs.add(new int[][]{dp[v][0], dp[v][1]}); // max profit[budget] with not buying v and with buying v
        }
        ...
        for (int parentBought = 0; parentBought <= 1; parentBought++) {
            ...
            base[0] = 0;
            for (int[][] child : childDPs) { // looping through all the profit array of childs.
